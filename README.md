...
- Clinical/Outcome:
  ...
  `response_status`, `mortality_30d`, `mortality_60d`, `mortality_90d`
...
$ docker run \
  ...
  ghcr.io/mdw-nl/datavalgen-model-example:v0.3.1 \
  validate
...
You can also use the same image to generate fake data to a mounted `data`
directory.

Generate the full fictional dataset:
```
$ mkdir -p data
$ docker run \
  --network none --rm --read-only --cap-drop=ALL --security-opt no-new-privileges=true --log-driver=none --user $(id -u):$(id -g) \
  -v ./data:/data \
  ghcr.io/mdw-nl/datavalgen-model-example:v0.3.1 \
  generate -f example -n 10 -o /data/generated_full.csv
Generated 10 rows to /data/generated_full.csv in csv format.
```
